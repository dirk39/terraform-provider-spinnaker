...
func runTask(client *gate.GatewayClient, task map[string]interface{}) error {
	ref, _, err := client.TaskControllerApi.TaskUsingPOST1(client.Context, task)
	if err != nil {
		return err
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return fmt.Errorf("Encountered an error processing task, status code: %d\n", resp.StatusCode)
	}
	if !taskSucceeded(task) {
		return fmt.Errorf("Encountered an error processing task, task output was: %v\n", task)
	}
